...
package com.example.cookie.ui.theme
...
private val LightColorPalette = CookieColors(
    brand = Shadow5,
...
private val DarkColorPalette = CookieColors(
    brand = Shadow1,
...
@Composable
fun CookieTheme(darkTheme: Boolean = isSystemInDarkTheme(), content: @Composable () -> Unit) {
    val colors = if (darkTheme) DarkColorPalette else LightColorPalette

    ProvideCookieColors(colors) {
        MaterialTheme(
...
object CookieTheme {
    val colors: CookieColors
        @Composable
        get() = LocalCookieColors.current
}

/**
 * Cookie custom Color Palette
 */
@Immutable
data class CookieColors(
...
@Composable
fun ProvideCookieColors(colors: CookieColors, content: @Composable () -> Unit) {
    CompositionLocalProvider(LocalCookieColors provides colors, content = content)
}

private val LocalCookieColors = staticCompositionLocalOf<CookieColors> {
    error("No CookieColorPalette provided")
}

/**
 * A Material [Colors] implementation which sets all colors to [debugColor] to discourage usage of
 * [MaterialTheme.colorScheme] in preference to [CookieTheme.colors].
 */
